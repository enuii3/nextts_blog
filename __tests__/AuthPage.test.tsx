...
describe('AdminPage Test Cases', () => {
  ...
  it('Should change a register mode', async () => {
    const { page } = await getPage({
      route: '/admin-page',
    })
    render(page)
    ...
    expect(await screen.getByText('Sign up')).toBeInTheDocument()
    expect(await screen.getByText('Create new user')).toBeInTheDocument()
  })
  ...
  it('Should not route to index-page when registration is faild', async () => {
    server.use(
      rest.post(
        `${process.env.NEXT_PUBLIC_RESTAPI_URL}/register/`,
        (req, res, ctx) => {
